      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 md:px-8 py-8 space-y-8">
        ...
        {generatedToken && (
          <div className="p-5 bg-emerald-950/60 border border-emerald-500/50 rounded-2xl space-y-3 animate-fadeIn">
            ...
            <div className="flex items-center gap-2 bg-[#0d1117] p-2.5 rounded-lg border border-[#30363d] font-mono text-xs">
              <input
                readOnly
                value={generatedToken}
                className="flex-1 bg-transparent text-cyan-300 focus:outline-none select-all"
              />
              <button
                onClick={handleCopy}
                className="px-3 py-1 bg-[#21262d] hover:bg-[#30363d] text-white rounded text-xs font-semibold flex items-center gap-1 transition-colors"
              >
        ...
        {/* Generate Token Box */}
        <form onSubmit={handleCreateToken} className="p-6 bg-[#161b22] border border-[#30363d] rounded-2xl space-y-4">
          ...
          <p className="text-xs text-[#8b949e]">
            ...
            <code className="block mt-1 text-cyan-400 font-mono">git clone http://username:TOKEN@localhost:4000/git/:owner/:repo.git</code>
          </p>

          <button
            type="submit"
            className="px-4 py-2 bg-[#238636] hover:bg-[#2ea043] font-semibold text-xs text-white rounded-lg transition-colors shadow-md shadow-emerald-900/20"
          >
            Generate token
          </button>
        </form>
        ...
        <div className="border border-[#30363d] rounded-2xl overflow-hidden bg-[#161b22]">
          <div className="p-3.5 bg-[#1c2128] border-b border-[#30363d] text-xs font-semibold text-[#8b949e]">
          ...
          <div className="divide-y divide-[#21262d]">
            {pats.map((pat, idx) => (
              <div key={idx} className="p-4 flex items-center justify-between gap-4 text-xs">
                ...
                <span className="text-[#8b949e]">Created {pat.date}</span>
              </div>
            ))}
          </div>
        </div>
